...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (user?.id) {
      console.log('User has ID, calling loadDashboardData');
      loadDashboardData();
    } else {
      console.log('User not loaded yet or no ID');
    }
  }, [user]);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load initial groups
      console.log('About to load random groups...');
      await loadRandomGroups();
      console.log('Finished loading random groups');
  ...
  const loadRandomGroups = async () => {
    try {
      console.log('Loading random groups for user:', user.id);
