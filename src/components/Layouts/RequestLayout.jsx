...
export default function RequestLayout({ children, title, subtitle }) {
  ...
  const sidebarNavItems = [
    { to: "/StudentConnect", label: "Dashboard", icon: "📊", type: "link" },
    {
      ...
      subItems: [
        ...
        { to: "/requests/my-requests/draft", label: "Draft Requests" },
        { to: "/requests/my-requests/active", label: "Active Requests" },
        { to: "/requests/my-requests/completed", label: "Completed" }
      ]
    },
    {
      to: "/OneToOneRequests",
  ...
  // Check if current path matches any create requests paths
  ...
  // Get active sidebar link styling
  const getSidebarLinkClass = (path, isSubItem = false) => {
    ...
    const isActive = location.pathname === path ||
      (path === '/requests/create' && location.pathname.startsWith('/requests/create')) ||
      (path === '/requests/my-requests' && location.pathname.startsWith('/requests/my-requests')) ||
      (path === '/OneToOneRequests' && location.pathname.startsWith('/OneToOneRequests'));

    return isActive
      ? `${baseClass} ${isSubItem ? 'bg-blue-25 text-blue-600 border-l-2 border-blue-600' : 'bg-blue-50 text-blue-600'}`
  ...
  // Handle sub-item click to close parent section
  const handleSubItemClick = (parentSetExpanded) => {
    // Close the parent expandable section when sub-item is clicked
    setTimeout(() => {
      parentSetExpanded(false);
    }, 100); // Small delay to allow navigation to complete
  };

  // Handle outside click to close expanded sections and navigate to dashboard
  const sidebarRef = useRef(null);

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (sidebarRef.current && !sidebarRef.current.contains(event.target)) {
        // Close all expanded sections when clicking outside sidebar
        if (createRequestsExpanded || receivedRequestsExpanded) {
          setCreateRequestsExpanded(false);
          setReceivedRequestsExpanded(false);
          // Navigate to dashboard when closing via outside click
          navigate('/StudentConnect');
        }
      }
    };
  ...
  }, [createRequestsExpanded, receivedRequestsExpanded, navigate]);

  // Handle escape key to close expanded sections and navigate to dashboard
  useEffect(() => {
    const handleEscape = (event) => {
      if (event.key === 'Escape') {
        if (createRequestsExpanded || receivedRequestsExpanded) {
          setCreateRequestsExpanded(false);
          setReceivedRequestsExpanded(false);
          // Navigate to dashboard when closing via escape key
          navigate('/StudentConnect');
        }
      }
    };
                  ...
                  <div className="mt-1 space-y-1 bg-gray-50 rounded-lg p-2 border-l-2 border-blue-200">
                    {item.subItems.map((subItem) => (
                      <Link
                        ...
                        onClick={() => handleSubItemClick(item.setExpanded)}
                      >
...
// Simple layout without sidebar for non-request pages
export function SimpleLayout({ children, className = "" }) {
  return (
    <div className="min-h-screen bg-gray-50">
      <IntelligentNavbar />
...
// ✅ COMPLETELY FIXED: Full width layout for chat pages - no conflicts
export function FullWidthLayout({ children }) {
  return (
    <div className="min-h-screen bg-gray-50">
      <IntelligentNavbar />
      {/* ✅ CRITICAL: NO wrapper divs, NO height restrictions, NO overflow hidden */}
      {/* Let the GroupChat component handle its own layout completely */}
      {children}
    </div>
  );
}
